...
```
airmcp-dev create <이름>              새 MCP 서버 프로젝트 생성
airmcp-dev add tool <이름>            도구/리소스/프롬프트 scaffold 추가
airmcp-dev dev [--console]            개발 모드 (핫 리로드 + 테스트 콘솔)
airmcp-dev start                      프로덕션 모드 (백그라운드)
airmcp-dev stop                       서버 중지
airmcp-dev status                     상태 확인
airmcp-dev list                       도구/리소스/프롬프트 목록
airmcp-dev inspect <도구>             도구 스키마 확인
airmcp-dev connect <클라이언트>       Claude Desktop, Cursor 등에 등록
airmcp-dev disconnect <클라이언트>    등록 해제
airmcp-dev check                      프로젝트 진단
```
