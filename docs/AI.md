...
```
airmcp-dev create <name> [--template basic|crud|api|agent] [--lang ko|en]
airmcp-dev add <type> <name> [--params "key:type,..."]
...
airmcp-dev connect <client> [-t transport] [-p port] [-H host] [--proxy path]
airmcp-dev disconnect <client>
airmcp-dev check
```

Supported clients: claude-desktop, claude-code, cursor, vscode, chatgpt, ollama, vllm, lm-studio

## Project Structure (generated by airmcp-dev create)
